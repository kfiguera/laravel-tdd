    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg p-4">
                <form action="{{ route('repositories.update',$repository) }}" method="post" class="sm:max-w-md">
                    @csrf
                    @method('PUT')
                    ...
                    <input type="text" name="url" class="form-input w-full rounded-md shadow-sm"
                           value="{{ old('url', $repository->url) }}">

                    <label for="" class="block font-medium text-sm text-gray-700">Descripcion *</label>
                    <textarea type="text" name="description"
                              class="form-input w-full rounded-md shadow-sm">{{ old('description', $repository->description) }}</textarea>
                    <hr class="my-4">
